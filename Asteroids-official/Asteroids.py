...
# Make surface and display
        ...
        self.gameDisplay = pygame.display.set_mode(
            (constant.Display.width, constant.Display.height))
...
class Asteroid:
    ...
    def updateAsteroid(self):
        ...
        # Check for wrapping
        if self.x > constant.Display.width:
            self.x = 0
        elif self.x < 0:
            self.x = constant.Display.width
        elif self.y > constant.Display.height:
            self.y = 0
        elif self.y < 0:
            self.y = constant.Display.height
        ...
        for v in range(len(self.vertices)):
            ...
            pygame.draw.line(gameDisplay, constant.Color.white, (self.x + this_v[0] * math.cos(this_v[1] * math.pi / 180),
...
class Bullet:
    ...
    def updateBullet(self):
        # Moving
        self.x += constant.BULLET_SPEED * math.cos(self.dir * math.pi / 180)
        self.y += constant.BULLET_SPEED * math.sin(self.dir * math.pi / 180)

        # Drawing
        pygame.draw.circle(gameDisplay, constant.Color.white, (int(self.x), int(self.y)), 3)
...
class Saucer:
    ...
    def updateSaucer(self):
        # Move player
        self.x += constant.SAUCER_SPEED * math.cos(self.dir * math.pi / 180)
        self.y += constant.SAUCER_SPEED * math.sin(self.dir * math.pi / 180)
        ...
        if self.y < 0:
            ...
            self.y = 0
        if self.x < 0 or self.x > constant.Display.width:
            self.state = "Dead"
    ...
    def createSaucer(self):
        ...
        # Set random position
        self.x = random.choice((0, constant.Display.width))
        self.y = random.randint(0, constant.Display.height)
    ...
    def drawSaucer(self):
        # Draw saucer
        pygame.draw.polygon(gameDisplay, constant.Color.white,
        ...
        pygame.draw.line(gameDisplay, constant.Color.white,
                         (self.x - self.size, self.y),
                         (self.x + self.size, self.y))
        pygame.draw.polygon(gameDisplay, constant.Color.white,
                            ((self.x - self.size / 2, self.y - self.size / 3),
...
class Player:
    ...
    def updatePlayer(self):
        ...
        if self.thrust:
            if speed + constant.FD_FRIC < constant.PLAYER_MAX_SPEED:
                self.hspeed += constant.FD_FRIC * math.cos(self.dir * math.pi / 180)
                self.vspeed += constant.FD_FRIC * math.sin(self.dir * math.pi / 180)
            else:
                self.hspeed = constant.PLAYER_MAX_SPEED * math.cos(self.dir * math.pi / 180)
                self.vspeed = constant.PLAYER_MAX_SPEED * math.sin(self.dir * math.pi / 180)
        else:
            if speed - constant.BD_FRIC > 0:
                change_in_hspeed = (constant.BD_FRIC * math.cos(self.vspeed / self.hspeed))
                change_in_vspeed = (constant.BD_FRIC * math.sin(self.vspeed / self.hspeed))
                if self.hspeed != 0:
    ...
    def killPlayer(self):
        # Reset the player
        self.x = constant.Display.width / 2
        self.y = constant.Display.height / 2
        self.thrust = False
        self.dir = -90
        self.hspeed = 0
        self.vspeed = 0
...
def gameLoop(startingState):
    ...
    playOneUpSFX = 0
    intensity = 0
    player = Player(constant.Display.width / 2, constant.Display.height / 2)
    saucer = Saucer()
    ...
    while gameState != "Exit":
        ...
        while gameState == "Menu":
            gameDisplay.fill(constant.Color.black)
            drawText("ASTEROIDS", constant.Color.white, constant.Display.width / 2, constant.Display.height / 2, 100)
            drawText("Press any key to START", constant.Color.white, constant.Display.width / 2, constant.Display.height / 2 + 100, 50)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LEFT:
                    player.rtspd = -constant.PLAYER_MAX_RTSPD
        ...
        if hyperspace != 0:
            ...
            hyperspace -= 1
            if hyperspace == 1:
                player.x = random.randrange(0, constant.Display.width)
                player.y = random.randrange(0, constant.Display.height)
        ...
        for a in asteroids:
            ...
            if player_state != "Died":
                if isColliding(player.x, player.y, a.x, a.y, a.size):
                    ...
                    player_pieces.append(deadPlayer(player.x, player.y, 5 * constant.PLAYER_SIZE / (2 * math.cos(math.atan(1 / 3)))))
                    player_pieces.append(deadPlayer(player.x, player.y, 5 * constant.PLAYER_SIZE / (2 * math.cos(math.atan(1 / 3)))))
                    player_pieces.append(deadPlayer(player.x, player.y, constant.PLAYER_SIZE))
        ...
        for f in player_pieces:
            f.updateDeadPlayer()
            if f.x > constant.Display.width or f.x < 0 or f.y > constant.Display.height or f.y < 0:
        ...
        if len(asteroids) == 0 and saucer.state == "Dead":
            if next_level_delay < 30:
                ...
                for i in range(stage):
                    xTo = constant.Display.width / 2
                    yTo = constant.Display.height / 2
                    while xTo - constant.Display.width / 2 < constant.Display.width / 4 and yTo - constant.Display.height / 2 < constant.Display.height / 4:
                        xTo = random.randrange(0, constant.Display.width)
                        yTo = random.randrange(0, constant.Display.height)
        ...
        if saucer.state == "Dead":
            ...
            acc = constant.SMALL_SAUCER_ACCURACY * 4 / stage
        ...
        if gameState != "Game Over":
            ...
            drawText("Game Over", constant.Color.white, constant.Display.width / 2, constant.Display.height / 2, 100)
            drawText("Press \"R\" to restart!", constant.Color.white, constant.Display.width / 2, constant.Display.height / 2 + 100, 50)
            live = -1

        # Draw score
        drawText(str(score), constant.Color.white, 60, 20, 40, False)
