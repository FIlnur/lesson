...
export class GameContainer {
  constructor(element, rows, cols, triesCount) {
    ...
    this._cardElements = [];
    this._guessedCounter = new Counter(0);
    ...
    this._guessedCounterElement = document.createElement("p");
    this._guessedCounterElement.className = "guessed-counter";
  ...
  appendDefaultChildren() {
    this.updateTriesCounter(this._remainingTries);
    this._element.appendChild(this._triesCounter);
    this.appenedCardsChildren();
    ...
    this._element.appendChild(this._guessedCounterElement);
  }
  ...
  onCardClick(event) {
    ...
    if (this._openedCards.length >= this._maxOpenedCount) {
      ...
      if (this._openedCards.every(isEqualToFirst)) {
        ...
        this._guessedCounter.increment(2);
        this._guessedCounterElement.textContent = this._guessedCounter.count;
      } else {
  ...
  static _generatePalette(count) {
    ...
    return new Array(count).fill(null).map(() => {
      const result = `hsl(${Math.floor(currentHue % 360)}, 80%, ${isEven ? "20" : "80"}% )`;
      currentHue += step;
      isEven = !isEven;
      return result;
    });
